...
export const WorkspaceViewWithAI: React.FC = () => {
  ...
  useEffect(() => {
    ...
    tabSync.setConflictCallback((conflictData) => {
      ...
      // Here you would show the conflict modal
    });
  ...
  const handleCommandResult = (command: string, result: string) => {
    ...
    // Here you would insert the result into the editor
    if (command.includes('总结') && activeNote) {
      setAISummary(result);
    }
  };
