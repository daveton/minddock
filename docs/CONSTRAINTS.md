...
- Sidebar、Note List、Editor、Context 四个区域都必须有最小可用宽度。
- 拖拽栏宽时只能应用鼠标移动增量，不得重复累加从起点开始的总偏移。
- 任一栏拉宽后，其他栏不得被挤出 viewport；窗口缩小时必须自动回收超出宽度。
- 弹层必须锚定明确入口：设置面板来自侧栏设置按钮，导出/数据操作来自右上更多菜单。
- 常用阅读态保持安静：底部编辑功能区可由顶部 `BIU` 显示或隐藏。
- 不再展示无功能的 macOS 红黄绿窗口标识；若恢复，必须绑定真实窗口语义或仅用于静态视觉稿。

代码维护约束：

- 主界面组件超过约 1,200 行时，应优先拆 UI-only 子组件；拆分不得改变编辑器输入路径。
- 样式文件超过约 1,500 行时，应按 surface 分区或拆分，但不能引入与当前构建系统不匹配的 CSS 框架。
- 删除已经没有 DOM 使用方的样式，避免原型残留影响像素判断。
